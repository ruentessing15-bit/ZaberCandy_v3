-- // Zaber candy🍬 [Full Integrated Script - Mobile Optimized]
...
pcall(function()
    Camera.MaxVisibleRadius = 9e9
    if settings().Rendering then
        settings().Rendering.DrawDistanceMax = 9e9
    end
end)
...
CombatTab:Slider({ Title = "FOV Radius", Step = 1, Value = {Min = 50, Max = 800, Default = 150}, Callback = function(v) FOVCircle.Radius = tonumber(v) end })
...
PlayerTab:Slider({ Title = "High Jump Power", Value = {Min = 20, Max = 100, Default = 70}, Step = 1, Callback = function(v) highJumpPower = tonumber(v); if highJumpEnabled then local char = LocalPlayer.Character if char then local hum = char:FindFirstChildOfClass("Humanoid") if hum then hum.JumpPower = math.clamp(highJumpPower, 0, maxJumpPower) end end end end })
...
PlayerTab:Slider({ Title = "Underground Depth", Value = {Min = 1, Max = 100, Default = 10}, Step = 1, Callback = function(v) SinkDepth = tonumber(v) end })
...
local function GetResolvedVelocity(target, root)
    ...
    if dt > 0.001 and dt < 1 then calculatedVel = (currentPos - data.LastPos) / dt end
    ...
    if engineVel.Magnitude > 150 or math.abs(engineVel.Y) > 150 or (engineVel - data.LastVel).Magnitude > 200 then
        if calculatedVel.Magnitude > 0.1 then realVel = calculatedVel else realVel = Vector3.zero end
    end
    if realVel.Magnitude > 350 then realVel = realVel.Unit * 350 end
...
pcall(function()
    ...
    if getupvalue then
        local success, val = pcall(function() return getupvalue(consume_stamina, 2) end)
        if success and val and val.sprint_bar then
            local SprintBar = val.sprint_bar
            OriginalSprintUpdate = SprintBar.update
            SprintBar.update = function(...)
                if infStamEnabled then return OriginalSprintUpdate(function() return 1 end) else return OriginalSprintUpdate(...) end
            end
        end
    end
    task.spawn(function()
        ...
        local ct = nil
        for _, obj in ipairs(getgc(true)) do if type(obj) == "table" and rawget(obj, "event") and rawget(obj, "func") then ct = obj break end end
...
local function GetColorFromRarity(itemObj)
    ...
    if rarity == "Common" then return Color3.fromRGB(255, 255, 255)
    elseif rarity == "UnCommon" then return Color3.fromRGB(2, 177, 17)
    elseif rarity == "Rare" then return Color3.fromRGB(1, 132, 255)
    elseif rarity == "Legendary" then return Color3.fromRGB(255, 160, 6)
    elseif rarity == "Epic" then return Color3.fromRGB(150, 2, 255)
    elseif rarity == "Omega" then return Color3.fromRGB(255, 6, 10)
...
local function CreateESP(player)
    ...
    player_drawings[player] = {
        ...
        name = draw("Text", {Visible = false, Color = Color3.new(1, 1, 1), Size = 10, Center = true, Outline = true, Text = player.Name}),
        dist = draw("Text", {Visible = false, Color = Color3.new(1, 1, 1), Size = 9, Center = true, Outline = true, Text = ""})
    }
end
...
local function getDroppedFolder()
    ...
    for _, name in ipairs(names) do local folder = workspace:FindFirstChild(name) if folder then return folder end end
    return nil
end
...
RunService.RenderStepped:Connect(function(delta)
    ...
    if speedEnabled and localChar and localRoot then
        ...
        if hum and hum.MoveDirection.Magnitude > 0 then localRoot.CFrame = localRoot.CFrame + hum.MoveDirection.Unit * speedMultiplier * delta * 60 end
    end
    ...
    if showFov then FOVCircle.Color = RainbowFOV and Color3.fromHSV(tick() % 5 / 5, 1, 1) or Color3.fromRGB(255, 105, 180) end
    TargetLine.Visible = false
    ...
    if SilentAimEnabled then
        ...
        if myHead and targetHead then
            ...
            if onScreen then
                ...
                if not showFov or distFromCenter <= FOVCircle.Radius then TracerLine.From, TracerLine.To, TracerLine.Visible = my2D, t2D, true else TracerLine.Visible = false end
    ...
    for player, drawings in pairs(player_drawings) do
        ...
        if Char and hum and head and root and localRoot and isAlive(player) then 
            ...
            else for _, d in pairs(drawings) do d.Visible = false end end
        else for _, d in pairs(drawings) do d.Visible = false end end  
    end

    local dropfolder = getDroppedFolder()
    for i, d in pairs(item_drawings) do if not i or not i.Parent or not _G.EnabledItemDrop then if d.box then d.box:Remove() end if d.name then d.name:Remove() end if d.dist then d.dist:Remove() end item_drawings[i] = nil end end
                    ...
                    if item.Name ~= "Money" then
                        ...
                        if ReplicatedStorage:FindFirstChild("Items") then
                            for _, x in pairs(ReplicatedStorage.Items:GetChildren()) do 
                                for _, b in pairs(x:GetChildren()) do if tostring(item.Name) == tostring(b.Name) then local rarityColor = GetColorFromRarity(b) itemesp.box.Color = rarityColor itemesp.name.Color = rarityColor colorFound = true break end end
                                if colorFound then break end
                            end
                        end
                        if not colorFound then itemesp.box.Color = Color3.fromRGB(255, 255, 255) itemesp.name.Color = Color3.fromRGB(255, 255, 255) end
                    else itemesp.box.Color = Color3.fromRGB(85, 255, 127) itemesp.name.Color = Color3.fromRGB(85, 255, 127) end
...
RunService.Heartbeat:Connect(function()
    ...
    if hrp and droppedItems then
        for _, item in pairs(droppedItems:GetChildren()) do
            ...
            if (hrp.Position - item.Position).Magnitude <= MagnetConfig.Radius then task.spawn(function() pcall(function() remoteGet:InvokeServer("pickup_dropped_item", item) end) end) end
        end
    end
end)
